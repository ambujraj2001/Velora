            ...
            <div className="max-w-4xl mx-auto py-10">
                <header className="mb-12">
                    ...
                    <h1 className="text-5xl font-black text-white tracking-tight mb-4">
                        Getting Started with <span className="text-transparent bg-clip-text bg-linear-to-r from-[#F06543] to-[#FF8A70]">Velora</span>
                    </h1>
                    <p className="text-xl text-[#666] leading-relaxed max-w-2xl">
                        Connect your analytical engines and start chatting with your data in minutes. 
                        Follow this guide to set up your first connection.
                    </p>
                </header>
                    ...
                    <section>
                        ...
                        <div className="grid md:grid-cols-3 gap-6">
                            {[
                                ...
                                <div key={i} className="p-6 rounded-2xl bg-[#111] border border-white/5 hover:border-[#F06543]/30 transition-all duration-300">
                            ...
                            <div className="p-8 border-b border-white/5 bg-linear-to-br from-white/5 to-transparent">
                                <div className="flex items-center justify-between mb-2">
                                    <div className="flex items-center gap-2 text-[#F06543]">
                                        <Database size={20} />
                                        <span className="text-xs font-black uppercase tracking-[0.2em]">Sample Connection</span>
                                    </div>
                            ...
                            <div className="p-0">
                                <div className="grid grid-cols-1 md:grid-cols-2">
                                    {credentials.map((cred, i) => (
                                        <div key={i} className={`p-6 border-white/5 flex flex-col gap-1 hover:bg-white/2 group transition-colors ${i % 2 === 0 ? 'md:border-r' : ''} ${i < 4 ? 'border-b' : ''}`}>

                                            <span className="text-[10px] font-black uppercase tracking-widest text-[#444] group-hover:text-[#666] transition-colors">{cred.label}</span>
                                            <div className="flex items-center justify-between gap-4">
                                                ...
                                                >
                                                    {copied === cred.label ? <Check size={14} className="text-green-500" /> : <Copy size={14} />}
                                                </button>
                                            </div>
                                        </div>
                                    ))}
...
ORDER BY (scheduled_dep, flight_id);`}
                        ...
                        <section className="flex flex-col h-full">
                            ...
                            <div className="flex-1 space-y-3">
                                {sampleQuestions.map((q, i) => (
                                    ...
                                    >
                                        <span className="text-sm text-[#888] group-hover:text-white transition-colors">{q}</span>
                                        {copied === `q-${i}` ? <Check size={14} className="text-green-500" /> : <ChevronRight size={14} className="text-[#333] group-hover:text-[#F06543]" />}
                                    </div>
                                ))}
                            </div>
                        </section>
                    ...
                    <footer className="mt-12 p-8 rounded-3xl bg-linear-to-br from-[#F06543]/10 to-transparent border border-[#F06543]/20 flex flex-col md:flex-row items-center justify-between gap-6">
                        <div>
                            ...
                            <p className="text-[#666] text-sm leading-relaxed">Our infrastructure team is available 24/7 for complex schema tuning.</p>
                        </div>
