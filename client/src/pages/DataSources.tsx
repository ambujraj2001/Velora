    ...
    <Layout>
      ...
      <div className="mb-8 rounded-xl border border-[#222] bg-[#141414] p-5">
        <div className="flex items-center gap-3">
            <div className="rounded-full bg-[#1A1A1A] p-2 text-[#444]"><Compass size={18} /></div>
      ...
      <section className="mb-10">
        <h2 className="mb-4 text-sm font-bold uppercase tracking-widest text-[#444]">Connected Sources</h2>
          ...
          {loading ? (
            ...
            <div className="divide-y divide-[#222]">
              {connections.map((conn) => (
                <div key={conn.id} className="flex items-center justify-between p-4 px-6 hover:bg-[#1A1A1A] group cursor-pointer">
      ...
      <section>
        <h2 className="mb-4 text-sm font-bold uppercase tracking-widest text-[#444]">Available Data Sources</h2>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {availableSources.map((source) => {
            ...
            return (
              ...
              >
                <div className={`rounded-lg bg-white/5 p-3 ${isSupported ? 'text-[#F06543]' : 'text-zinc-600'}`}>
                  {source.icon}
                </div>
                <div className="flex flex-col">
                  <span className={`font-semibold ${isSupported ? 'text-white' : 'text-zinc-500'}`}>{source.type}</span>
                  {!isSupported && <span className="text-[10px] uppercase font-bold tracking-widest text-zinc-600">Coming Soon</span>}
                </div>
                {!isSupported && (
                  <div className="absolute inset-0 flex items-center justify-center rounded-xl bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity">
                     <span className="text-[11px] font-bold uppercase tracking-widest text-white px-3 py-1 bg-[#222] rounded-full">Coming Soon</span>
                  </div>
                )}
              </button>
            );
